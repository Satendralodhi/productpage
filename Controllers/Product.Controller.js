...
getProductsbyId:async(req,res,next)=>{
        ...
        const results= await Product.findAll({
            where:{productId},
            include:[SEO_Model,Shipping_Model,Dimension_Model,Ratings_Model,Discount_Model,video_Model,Image_Model]})
...
// create product post api
...
else if (req.body.ProductTableProductId!=null &&(req.body.rating!=null||req.body.review!=null)){
    const ratingT= await Ratings_Model.create({
...
else if (req.body.ProductTableProductId!=null &&req.body.images!=null){
    const imageT=await Image_Model.create({
        productId:req.body.ProductTableProductId,
        ProductTableProductId :  req.body.ProductTableProductId,
        images:req.body.images
    ...
    else if (req.body.ProductTableProductId!=null &&req.body.videos!=null){
        const videoT=await video_Model.create({
            productId:req.body.ProductTableProductId,
            ProductTableProductId :  req.body.ProductTableProductId,
            videos:req.body.videos
    ...
    else if (req.body.ProductTableProductId!=null &&(req.body.type||req.body.value)){
    ...
    const { name,description,price,brand,category,subCategory,
        color,size, gender,attributes,features,availability,quantity,sold,tags,inventoryManagement
        ,relatedProducts,customizationOptions,metaTitle,metaDescription,metaKeywords,type,value,rating,review
        ,weight,restriction,length,width,height,images,videos}=req.body
    ...
    await Product.update({
        ...
        inventoryManagement ,
        relatedProducts ,
        customizationOptions},{where:{productId},transaction});
        ProductTableProductId=productId
        await Shipping_Model.update({ weight,restriction},{where:{productId},transaction})
        await Dimension_Model.update({ length, width,height },{where:{ProductTableProductId},transaction});
        await Discount_Model.update({type,value},{where:{productId},transaction})
        ...
        await SEO_Model.update({   metaTitle ,metaDescription,metaKeywords},{where:{ProductTableProductId},transaction})
